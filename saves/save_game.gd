...
func update_variables(game_save: Dictionary) -> void:
	...
	for level in levels:
		...
		for interactable_object in level_data["interactables"].keys():
			...
			new_interactable_object.get_child(new_interactable_object.get_child_count() - 1).value = interactable_object_data["$Ignore Meter"].value
			...
			new_interactable_object["actions"] = load("res://actions/action_layout.tscn").instantiate() as Control
			...
			for i in range(new_interactable_object["actions"]["number_of_actions"]):
				...
				new_interactable_object["actions"]["actions"][i].get_child(0).get_child(0).get_child(1).text = interactable_object_data["actions"]["actions_text"][i]
