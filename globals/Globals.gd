...
# Player Related
var stats: Dictionary = {"profit": -0.5, 
	"public_relations": -0.5, "employee_friendliness": -0.5, 
	"product_quality": -0.5, "environment_indicators": -0.5
}
...
func add_value_to_stat(stat: String, value: float) -> void:
	...
	stats[stat] += value
	stats[stat] = clamp(stats[stat], -0.5, 1.5)
